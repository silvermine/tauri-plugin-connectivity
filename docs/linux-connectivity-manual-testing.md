...
The Linux implementation has two runtime paths:
   ...
   * Passive fallback using `/proc/net/route`, `/proc/net/ipv6_route`, and
     `/sys/class/net`. This path is used when the system bus is unavailable,
     NetworkManager is not running, or a NetworkManager query fails.
...
| Local-only `none` | VirtualBox host-only adapter | Tested | disconnected |
| NetworkManager limited | VirtualBox host-only adapter | Tested | `connected: true`, `constrained: true` |
...
```text
constrained = network_manager_connectivity_is_portal_or_limited
...
| NetworkManager `Connectivity` | `2`, `3` | Connected and constrained |
| NetworkManager `Connectivity` | `1` | Disconnected |
...
Expected connected fallback response when a non-loopback IPv4 or IPv6 default
route exists:
...
If NetworkManager reports `none`, the plugin should return disconnected:
...
If NetworkManager reports `limited`, the plugin should return connected and
constrained:
